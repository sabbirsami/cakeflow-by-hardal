...
const depth = 10; // px vertical offset between layers
...
function shapeDef(shape: CakeShapeId): ShapeDef {
  switch (shape) {
    ...
    case 'square':
      return { tag: 'rect', attrs: { x: 40, y: 35, width: 120, height: 70, rx: 12, ry: 12 } } as any
    case 'rectangle':
      return { tag: 'rect', attrs: { x: 30, y: 50, width: 140, height: 50, rx: 10, ry: 10 } } as any
    case 'hexagon':
      return { tag: 'polygon', attrs: { points: '100,30 145,55 145,95 100,120 55,95 55,55' } } as any
    case 'star':
      return { tag: 'polygon', attrs: { points: '100,30 118,62 154,66 128,90 135,126 100,108 65,126 72,90 46,66 82,62' } } as any
    case 'heart':
      return { tag: 'path', attrs: { d: 'M100 52 C 112 36, 136 36, 148 52 C 162 70, 152 94, 100 120 C 48 94, 38 70, 52 52 C 64 36, 88 36, 100 52 Z' } } as any
    default:
      return { tag: 'rect', attrs: { x: 40, y: 35, width: 120, height: 70, rx: 12, ry: 12 } } as any
  }
}
...
export function ShapePreview({ shape, layers = 1, className }: ShapePreviewProps) {
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: L }).map((_, idx) => {
        ...
        const y = i * depth;
        const topFill = layerFill(i, L);
        const sideFill = layerSideFill(i, L);
        return (
          <g key={i}>
